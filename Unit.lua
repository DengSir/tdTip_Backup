...
local Unit = Addon:NewModule('Unit', 'AceHook-3.0', 'AceEvent-3.0', 'AceTimer-3.0')
...
function Unit:ClearLine(i)
    tipleft[i]:SetText()
end

function Unit:SetLine(i, text, r, g, b, a)
    if tipleft[i] then
...
function Unit:SetIcon(texturePath)
    if texturePath then
...
function Unit:UpdateTarget(force)
...
function Unit:OnTooltipSetUnit(tip)
...
function Unit:Tooltip_OnSizeChanged(tip, w, h)
    GameTooltipStatusBar:SetWidth(w - 20)
end

function Unit:Tooltip_OnHide()
...
function Unit:StatusBar_OnShow(bar)
...
function Unit:StatusBar_OnValueChanged(bar, value)
